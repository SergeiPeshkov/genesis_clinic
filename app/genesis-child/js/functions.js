...
(function ($) {
  ...
  SEMICOLON.initialize = {
    ...
    lightbox: function () {
      ...
      if (window.location.pathname.match(/^\/gallery/)) {
        if ($lightboxGalleryEl.length > 0) {
          $lightboxGalleryEl.on("mfpOpen", function () {
          ...
          function clickLightboxOpenButton(currClickTarget) {
            $(currClickTarget)
              .parents(".iportfolio")
              .find(
                '.portfolio-image a.center-icon[data-lightbox="gallery-item"]'
              )
              .trigger("click");
          }
          $(".portfolio-desc h3 a").click((e) => {
            //TODO: portfolio-desc_video /public_html/wp-content/themes/genesis-child/archive-gallery.php
            e.preventDefault();
            clickLightboxOpenButton(e.target);
          });
          $(".portfolio-image .portfolio-overlay").click((e) => {
            e.preventDefault();
            clickLightboxOpenButton(e.target);
          });
        }
  ...
  SEMICOLON.widget = {
    init: function () {
      ...
      if (/\/(answers|reviews)/.test(window.location.pathname)) {
              ...
              success: function (data) {
                ...
                if (elementAlert == "inline") {
                  ...
                  elementResult
                    ...
                    .html(data.message || "Запрос успешно отправлен")
                    .slideDown(400);
...
// это поиск на странице single-klinik.html
$(document).ready(function () {
  class SearchRegExp {
    constructor(string) {
      ...
      let regexBody = [];
      const words = string.replace(/\s+/g, " ").replace(".", "\\.").split(" ");
      words.forEach((word) => {
        let letters = getLetters(word),
          wordRegexBody;
        if (word.length > 4) {
          wordRegexBody = letters
            .map((letter, i) => {
              let lettersLocal = letters.slice();
              lettersLocal[i] = ".";
              return lettersLocal.join("");
            })
            .join("|");
        } else {
          wordRegexBody = word;
        }
        regexBody.push(wordRegexBody);
      });
    ...
    check(string) {
      return this.body.every((word) => {
        let regex = new RegExp(word, "i");
        return regex.test(string);
      });
    }
  }
  window.SearchRegExp = SearchRegExp;

  //текстовое поле поиска
  const $searchField = $(".js-search-block");

  //Объекты этого класса хранят раскрываемые блоки и их содержимое
  class SearchResultBlock {
    constructor(wrapper, wrapperTitle, innerResults) {
  ...
  //Массив всех результатов будем хранить в глобальном объекте
  /**
   * @type Array<SearchResultBlock>
   */
  window.searchResultBlocks = [];

  //Выбираем все раскрываемые блоки
  $(".blocks .block-head").each(function () {
    const $wrapper = $(this);
    const $wrapperTitle = $($wrapper[0].querySelector(".block-head__title"));
    const $innerResults = Array.from(
      $wrapper[0].querySelectorAll(".block-head__shortname tr")
    );
    searchResultBlocks.push(
      new SearchResultBlock($wrapper, $wrapperTitle, $innerResults)
    );
  });
  if (searchResultBlocks.length == 0) {
    searchResultBlocks.push(
      new SearchResultBlock(
        null,
        null,
        Array.from(document.querySelectorAll(".block-head__shortname tr"))
      )
    );
  }
  ...
  function onSearchKeyUp() {
    ...
    const $value = $searchField.val().replace(/[ё]/gi, "е").trim();
    const isValNull = $value == "";
    const regex = new SearchRegExp($value);

    const fadeSpeed = "slow";

    //ищем в каждом блоке
    for (const searchBlock of searchResultBlocks) {
      let anyResultFound = false;
      //проверяем в каждом подблоке
      for (const targetResult of searchBlock.innerResults) {
        const $this = $(targetResult);
        if (regex.check($this.text().replace(/ё/gi, "е"))) {
          $this.fadeIn(fadeSpeed);
          anyResultFound = true;
        } else $this.fadeOut(fadeSpeed);
      }
      if (searchBlock.wrapper && searchBlock.wrapperTitle) {
        //если есть хоть одно совпадение и поле поиска не пустое, то показываем блок
        if (anyResultFound && !isValNull) {
          searchBlock.wrapper.fadeIn(fadeSpeed);
          $(searchBlock.wrapperTitle)
            .addClass("toggleta")
            .next(".togglec")
            .slideDown(300);
        } else if (isValNull) {
          //если же поле поиска пустое то схлопываем но показываем
          searchBlock.wrapper.fadeIn(fadeSpeed);
          $(searchBlock.wrapperTitle)
            .removeClass("toggleta")
            .next(".togglec")
            .slideUp(300);
        } else {
          //иначе схлопываем и не показываем
          searchBlock.wrapper.fadeOut(fadeSpeed);
          $(searchBlock.wrapperTitle)
            .removeClass("toggleta")
            .next(".togglec")
            .slideUp(300);
...
$(document).ready(function () {
  $(".js-search-block-lab").keyup(function () {
    var $value = $(this).val();
    $(".blocks-lab .block-lab").each(function (index) {
      $row = $(this);
      var animal_name = $row.find(".block-title-lab").html();
      var animaL_eng_name = $row.find(".block-short-lab").html();
...
// читать далее для описания клиник и прочего.
function bindReadMore(baseSelector, maxReviewLength) {
  jQuery(baseSelector).each(function () {
    let $readMore = jQuery(this),
      /*$review = $readMore.parents('.text_review');*/
	  $review = $('.text_review');
	
    if ($review.text().length <= maxReviewLength) {
      $readMore.hide();
      $review.css('height', '');
	$('.text_review__background').remove();
      return;
    }
    $readMore.click(e => {
      ...
      $review.css('height', '');
	   ...
	   //прокрутка вверх
	    $('html, body').animate({
        scrollTop: $('#content').offset().top  // класс объекта к которому приезжаем
...
bindReadMore('.text-review__read-more', 8000);
// bindReadMore('.text_review2', 1200);

/**
 * Фильтрация на странице специалистов
 */
$(document).ready(() => {
  //specDropdown, clinicDropdown, surnameTextField, posts появились благодаря указанным id

  try {
    specDropdown && clinicDropdown && surnameTextField && posts;
  } catch (err) {
    return;
  }

  const filter = function () {
    const specId = Array.from(specDropdown.querySelectorAll("option"))
      .map((option) => option.textContent)
      .indexOf(specDropdown.value);
    const clinicId = Array.from(clinicDropdown.querySelectorAll("option"))
      .map((option) => option.textContent)
      .indexOf(clinicDropdown.value);
    const surname = surnameTextField.value.trim();

    $(posts).isotope({
      filter: function () {
        return isTargetDoctor(this, specId, clinicId, surname);
      },
    });
  };

  $(specDropdown).change(filter);
  $(clinicDropdown).change(filter);
  $(surnameTextField).on("keyup", filter);

  /**
   * Check if doctor's block is match params
   * @param {HTMLElement} doctorBlock doctor's block element
   * @param {Number} specialization index of doctor's specialization
   * @param {Number} clinic index of doctor's clinic
   * @param {String} surname surname of doctor
   */
  function isTargetDoctor(
    doctorBlock,
    specialization = null,
    clinic = null,
    surname = null
    ...
    const surnameRegex = new SearchRegExp($value);

    const doctor = {
      surname: $(doctorBlock).find(".portfolio-desc > h3").text(),
      spec: doctorBlock.dataset.spec,
      clinic: doctorBlock.dataset.clinic ? doctorBlock.dataset.clinic.split(',') : [],
    };

    if (
      (specialization == 0 || doctor.spec == specialization) &&
      (clinic == 0 || doctor.clinic.indexOf(clinic.toString()) > -1) &&
      (!surname || surnameRegex.check(doctor.surname))
    ) {
      return true;
    } else return false;
  }
 ...
 */
 $(document).ready(() => {
  const $phones = $(".header-phones"),
    $toggle = $(".header-phones__toggle", $phones),
    $main = $(".header-phones__main", $phones),
    $more = $(".header-phones__more", $phones);
