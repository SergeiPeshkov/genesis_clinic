...
function load_style_script()
{
	...
	wp_enqueue_style('style.css', get_stylesheet_directory_uri() . '/style.css?v=5');
...
function trim_chars($t, $count, $chars) {
	$excertp = get_the_excerpt($t);
	if (mb_strlen($excertp) > $count){
		$excertp = mb_substr($excertp, 0, $count);
	}
	else {
		$t = ''; 
		$chars='..';
	}	
	return $excertp.''.$chars;
}