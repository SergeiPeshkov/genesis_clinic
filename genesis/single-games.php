...
<?php 
    $meta_key = in_category(18) ? 'dlc' : 'game' ;
    $ts = get_posts(array(
        'post_type' => 'trophies',
        'meta_key' => $meta_key,
        'meta_value' => $post->ID,
    ...
    $bronze_trophie  = 0;
    $unknown_trophie  = 0;
    $tr_page = get_post_meta($post->ID, 'tr_page', 1);
    foreach ($ts as $t) {
        $type = get_post_meta($t->ID, 'tr_class', 1);
        $$type++;
    }
    if ($platinum_trophie || $gold_trophie || $silver_trophie || $bronze_trophie || $unknown_trophie) {
?> 
<div class="gamedata">     
    <strong>Трофеи:&nbsp;</strong><br>
    Платина - <?=$platinum_trophie?><br>
    Золото - <?=$gold_trophie?><br>
    Серебро - <?=$silver_trophie?><br>
    Бронза - <?=$bronze_trophie?><br>
    <a href="<?=get_permalink($tr_page)?>">Перейти на страницу трофеев</a>
</div>
<?php } ?>
